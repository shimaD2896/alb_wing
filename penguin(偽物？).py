...
for k in range(1, n + 1):  # range(1,n+1):				 	#根から k 枚目のリブ
    ...
    if zenntanndome:
        kouenn_doffset = inter(dline_x, dline_y)
        #後縁前端棒止め
        ht = 5
        htl = 5.1
        Camberu_offsetKs = offset(CamberPs, ht/2, 1)
        dome_uOffset = inter(to_numpy_x(Camberu_offsetKs), to_numpy_y(Camberu_offsetKs))

        DomeD_x = optimize.newton(lambda x: dome_uOffset(x) - kouenn_doffset(x), c * cos(sweep) * 0.97)
        DomeD = vector(DomeD_x, kouenn_doffset(DomeD_x))


        Camberd_offsetKs = offset(CamberPs, ht/2, 0)
        dome_dOffset = inter(to_numpy_x(Camberd_offsetKs), to_numpy_y(Camberd_offsetKs))


        DomeU_x = optimize.newton(lambda x: dome_dOffset(x) - f_u(x), c * cos(sweep) * 0.97)
        DomeU = vector(DomeU_x, dome_dOffset(DomeU_x))
