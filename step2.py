...
tannribuHokyouDensity = 0.0001349  # 端リブ補強材（バルサ＋ボンド）の密度（g / mm²）
ribCapDensity = 0.0001349  # リブキャップの密度（g/リブキャップ１mm²）
...
# 既知の値
lengthOfKeta = 18900  # 桁の長さ
numberOfRyoumennteapForVerticalForYokugenn = 5  # 翼弦に対して垂直な方向の両面テープ数
sutairoDensity = 0.000031  # スタイロの密度(g/mm3)
ketaLengthFrangeinsideToFrangeInside = lengthOfKeta # 桁長さ
...
kouenzaisenmitudo = 0.02961805556 # 後縁材の線密度（g/mm）
stringerfixnum = 0.1833333333 # リブ1枚、ストリンガー１本当たりのストリンガー固定接着剤の重さ（g）
"""
...
readingFilePath = r"C:\Users\islan\OneDrive - OUMail (Osaka University)\ribwriting\重量推定root_28_ver1.xlsx"
yokuNumber = "0" 
...
def ribuWeight():  # リブのスタイロの部分の重量
    ...
    for ribData in ribuTotalData:  # リブの体積を計算する
        if ribData[7] == 1:  # 肉抜きを行う場合
            ...
            totalVolumeOfRib += volumeOfRib
        else:  # フルリブの場合端リブ厚さ2倍だから2倍にしてるごめん
            volumeOfRib = 2 * ribData[0] * ribData[8]
            totalVolumeOfRib += volumeOfRib
...
def weightOfstringerfix():  # ストリンガーの接着剤重量を計算する
    stringerFixWeight = (
        numberOfRib
        * NumberOfStringer
        * stringerfixnum
    )
    return stringerFixWeight
...
def weightOfKoennzai():  # 後縁材の重量を求める
    ...
    """
    #後縁材の線密度 × 桁の長さ
    return kouenzaisenmitudo * ketaLengthFrangeinsideToFrangeInside
...
totalWeightOfStringerFix = weightOfstringerfix()
...
df = pd.DataFrame(
    {
        ...
        "ストリンガー接着剤の重量(g)": [totalWeightOfStringerFix],
