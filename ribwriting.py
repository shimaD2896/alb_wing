...
# 前縁側上面
nikunukiBasePoint_u_Zenenn = [6, 11, 13, 15, 24, 26]

##前縁側下面
nikunukiBasePoint_d_Zenenn = [8.5, 10.5, 18.5, 20.5, 22.5, 31.5]

# 後縁側d1が動く設定になっている
nikunukiBasePoint_u_Kouenn = [47.5, 49.5, 58, 60, 62, 69, 71]

# 後縁側下面
nikunukiBasePoint_d_Kouenn = [43, 52.5, 54.5, 56.5, 64, 66, 68, 75]

# 各肉抜きを行うための基準点の翼厚みに対する移動距離(桁穴よりも前縁側)
sannkakunukunuki_base_move_y_u_zenenn = [-0.50, -0.20, -0.20, -0.20, -0.20, -0.20]
sannkakunukunuki_base_move_y_d_zenenn = [0.20, 0.20, 0.20, 0.20, 0.20, 0.20]


## 各肉抜きを行うための基準点の翼厚みに対する移動距離(桁穴よりも後縁側)
sannkakunukunuki_base_move_y_u_kouenn = [-0.20, -0.20, -0.20, -0.20, -0.20, -0.20, -0.20]
sannkakunukunuki_base_move_y_d_kouenn = [0.20, 0.20, 0.20, 0.20, 0.20, 0.20, 0.23, 0.25]

# ハーフリブを利用する際の肉抜き
nikunukiBasePoint_half_u_Kouenn_half = [46, 55.5]
nikunukiBasePoint_half_d_Kouenn_half = 43
# 各肉抜き点のy座標方向の移動
sannkakunukunuki_base_move_y_u_kouenn_half = [-0.15, -0.15]
sannkakunukunuki_base_move_y_d_kouenn_half = 0.32
...
for k in range(1, n + 1):  # range(1,n+1):				 	#根から k 枚目のリブ
    ...
    if use_half:
        sannkakunikunuki_for_halfRib_kouenn = makeSannkakuNikunukiObject(
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_half_u_Kouenn_half[0],
                sannkakunukunuki_base_move_y_u_kouenn_half[0],
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_half_u_Kouenn_half[1],
                sannkakunukunuki_base_move_y_u_kouenn_half[1],
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_half_d_Kouenn_half,
