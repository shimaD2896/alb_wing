...
file.write("-lweight\n0.001\n")  # 線の太さ設定
width =[]
length_normal = math.sqrt((((span * 2 / (n - 1)) - ribwidth_normal))**2 + ((RootChord - EndChord)*2*(1-RootR)/n)**2)
length_end = math.sqrt(((span / (n - 1)) -(ribwidth_end + (ribwidth_normal/2)))**2 + ((RootChord - EndChord)*(1-RootR)/n)**2)
...
for k in range(1, n + 1):  # range(1,n+1):				 	#根から k 枚目のリブ
    ...
    if ribcapddome:
        ...
        FoilD =[FoilD[i] for i in range(len(FoilD) -2 ) if (FoilD[i].x > x_stringer_D1) and (FoilD[i].x < realTrailD_x)]
    ...
    if k not in halfRibNumber:
        width.append(abs(stringerD2ToVec.D - stringerU1ToVec.D))
        print(length_end)
print(width[len(width)-1])
for k in range(0,len(width)-1):
    O.y += width[len(width)-2] + 2
    web_A = vector(0, width[k]/2)
    web_B = vector(0, -width[k]/2)
    if k in (0, len(width)-2):
        web_C = vector(length_end, width[k+1]/2)
        web_D = vector(length_end, -width[k+1]/2)
    else:
        web_C = vector(length_normal, width[k+1]/2)
        web_D = vector(length_normal, -width[k+1]/2)
    color(file, 255,255, 0) 
    line(file, web_A, web_B, O)
    line(file, web_B, web_D, O)
    line(file, web_D, web_C, O)
    line(file, web_C, web_A, O)
