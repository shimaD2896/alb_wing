...
# 端、根の翼弦長(流れ方向)[mm]
# 0、1翼
RootChord_original = 1000
EndChord_original = 1000
# 2翼
...
# 4翼
#rootstringerURate = [4,10,30,57.5]
#endstringerURate = [4,10,30,57.5]
# 5翼
#rootstringerURate = [4,10,15,30,57.5]
#endstringerURate = [4,10,15,30,57.5]

# ストリンガー位置翼下部[%]
# 0,1,2,3翼
rootstringerDRate = [4.5,9]
endstringerDRate = [4.5,9]
# 4翼
#rootstringerDRate = [10,15]
#endstringerDRate = [10,15]
# 5翼
#rootstringerDRate = [10,15]
#endstringerDRate = [10,15]
...
# 以下は翼根側の設定
# 前縁側上面
rootnikunukiBasePoint_u_Zenenn = [5.5, 11, 12.9, 15, 24, 26]
# 前縁側下面
rootnikunukiBasePoint_d_Zenenn = [8, 10.5, 18, 20.5, 22.5, 31.5]
# 後縁側上面
rootnikunukiBasePoint_u_Kouenn = [47, 49.8, 58, 60.3, 63.15, 69.3, 71.05, 73, 76, 77.5]
# 後縁側下面
rootnikunukiBasePoint_d_Kouenn = [43, 51.7, 54.2, 56.8, 64.3, 66.5, 68.7, 72.65, 74.53, 76.5, 78.5]
...
rootsannkakunukunuki_base_move_y_u_kouenn = [-0.14, -0.20, -0.20, -0.20, -0.20, -0.25, -0.20, -0.25, -0.3, -0.3]
rootsannkakunukunuki_base_move_y_d_kouenn = [0.16, 0.18, 0.18, 0.20, 0.20, 0.20, 0.23, 0.25, 0.25, 0.3, 0.3]
# ハーフリブを利用する際の肉抜き
rootnikunukiBasePoint_u_Kouenn_half = [46, 55.5]
rootnikunukiBasePoint_d_Kouenn_half = 43
# 各肉抜き点のy座標方向の移動
rootsannkakunukunuki_base_move_y_u_kouenn_half = [-0.15, -0.15]
rootsannkakunukunuki_base_move_y_d_kouenn_half = 0.32

# 以下は翼端側の設定
# 前縁側上面
endnikunukiBasePoint_u_Zenenn = [5.5, 11, 12.9, 15, 24, 26]
# 前縁側下面    
endnikunukiBasePoint_d_Zenenn = [8, 10.5, 18, 20.5, 22.5, 31.5]
# 後縁側上面
endnikunukiBasePoint_u_Kouenn = [47, 49.8, 58, 60.3, 63.15, 69.3, 71.05, 73, 76, 77.5]
# 後縁側下面
endnikunukiBasePoint_d_Kouenn = [43, 51.7, 54.2, 56.8, 64.3, 66.5, 68.7, 72.65, 74.53, 76.5, 78.5]
# 各肉抜きを行うための基準点の翼厚みに対する移動距離(桁穴よりも前縁側)
...
# 各肉抜きを行うための基準点の翼厚みに対する移動距離(桁穴よりも後縁側)
endsannkakunukunuki_base_move_y_u_kouenn = [-0.14, -0.20, -0.20, -0.20, -0.20, -0.20, -0.20, -0.20, -0.24, -0.3]
endsannkakunukunuki_base_move_y_d_kouenn = [0.16, 0.18, 0.20, 0.20, 0.20, 0.20, 0.23, 0.25, 0.25, 0.3, 0.3]
# ハーフリブを利用する際の肉抜き
endnikunukiBasePoint_u_Kouenn_half = [46, 55.5]
endnikunukiBasePoint_d_Kouenn_half = 43
# 各肉抜き点のy座標方向の移動
endsannkakunukunuki_base_move_y_u_kouenn_half = [-0.15, -0.15]
endsannkakunukunuki_base_move_y_d_kouenn_half = 0.32
...
for k in range(startRib , endRib + 1):  # range(1,n+1):				 	#根から k 枚目のリブ
    ...
    # 後縁材の出力
    realTrailD_x = optimize.newton(lambda x: (c - x)**2 + f_d(x)**2 - penguinlength_upper**2, c * cos(sweep) * 0.95)
    realTrailD = vector(realTrailD_x, f_d(realTrailD_x))
    realTrailDline = vector(realTrailD_x - c, f_d(realTrailD_x))
    realTrailU = realTrailD + realTrailDline.rotate(-90, "degree") * 0.2
    ...
    nikunukiBasePoint_d_Kouenn_half = rootnikunukiBasePoint_d_Kouenn_half + (endnikunukiBasePoint_d_Kouenn_half - rootnikunukiBasePoint_d_Kouenn_half) * r
    ...
    sannkakunukunuki_base_move_y_d_kouenn_half = rootsannkakunukunuki_base_move_y_d_kouenn_half + (endsannkakunukunuki_base_move_y_d_kouenn_half - rootsannkakunukunuki_base_move_y_d_kouenn_half) * r
    ...
    if k not in halfRibNumber: 
        ...
        sankakkeiObject_10 = makeSannkakuNikunukiObject(
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_u_Kouenn[7], sannkakunukunuki_base_move_y_u_kouenn[7]
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_u_Kouenn[8], sannkakunukunuki_base_move_y_u_kouenn[8]
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_d_Kouenn[8], sannkakunukunuki_base_move_y_d_kouenn[8]
            ),
        )
        sankakkeiObject_11 = makeSannkakuNikunukiObject(
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_u_Kouenn[9], sannkakunukunuki_base_move_y_u_kouenn[9]
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_d_Kouenn[9], sannkakunukunuki_base_move_y_d_kouenn[9]
            ),
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_d_Kouenn[10], sannkakunukunuki_base_move_y_d_kouenn[10]
            ),
        )
        ...
        makeSannkakuNinuki(file, sankakkeiObject_10)
        makeSannkakuNinuki(file, sankakkeiObject_11)
    ...
    if k  in halfRibNumber:
        sannkakunikunuki_for_halfRib_kouenn = makeSannkakuNikunukiObject(
            ...
            convertYokugennRateGaishuuyohakuToZahyou(
                nikunukiBasePoint_d_Kouenn_half,
                sannkakunukunuki_base_move_y_d_kouenn_half,
            ),
        )
        ...
        print(Plankend_u,halfRibCutLine_d)
        line(file,
            Plankend_u, 
